...
```bash
export MEUFOCO_CLI_GITHUB_TOKEN=<token>
curl -H "Authorization: token ${MEUFOCO_CLI_GITHUB_TOKEN}" -fsSL https://raw.githubusercontent.com/widia-io/widia-omni/main/scripts/install-cli.sh | bash
```

Observação: variáveis legadas (`WIDIA_CLI_*`) também funcionam por compatibilidade.

Esse comando:
- detecta seu sistema (Linux/macOS, amd64/arm64),
- consulta a release mais recente do GitHub,
- baixa o binário correto e instala em `/usr/local/bin/meufoco`,
- valida exibindo a versão do CLI instalado.

### Variáveis de ambiente

- `MEUFOCO_CLI_VERSION`: define a versão (ex.: `v1.0.0`).
- `MEUFOCO_CLI_INSTALL_DIR`: altera o diretório de instalação (padrão `/usr/local/bin`).
- `MEUFOCO_CLI_GITHUB_TOKEN`: token para repositórios privados.
...
- `meufoco-cli_<versao>_<os>_<arch>.tar.gz` para Linux e macOS,
- `meufoco-cli_<versao>_windows_<arch>.zip` para Windows.

Exemplos:
- `meufoco-cli_1.0.0_linux_amd64.tar.gz`
- `meufoco-cli_1.0.0_darwin_arm64.tar.gz`
- `meufoco-cli_1.0.0_windows_amd64.zip`
